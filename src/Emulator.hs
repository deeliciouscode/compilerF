...
-- Emulator
type Context = (Code, Stack, GlobalEnvironment, Heap, T, P)
...
emulate' (env, code) = emulate (code, [], env, [], 0, 0)

emulate :: Context -> Result
emulate all@(code, stack, global, heap, t, p)
...
fillHeap (code, stack, global, heap, t, p) = (code, stack, global, filledHeap, t, p)
                                where
...
execute all@(code, stack, global, heap, t, p) =
...
readCode :: Code -> P -> Instruction
readCode code p = code !! p

increaseP :: Context -> Context
increaseP (code, stack, global, heap, t, p) = (code, stack, global, heap, t, p')
                                where p' = p + 1

execReset :: Context -> Context
execReset (code, stack, global, heap, t, p) = (code, stack, global, heap, t', p)
                                where
                                    t' = -1

execPushfun :: String -> Context -> Context
execPushfun name (code, stack, global, heap, t, p) = (code, stack', global, heap, t', p)
                                where
                                    t' = t + 1
...
execCall pass@(code, stack, global, heap, t, p) =
    case getAdr heap stack t of
        (DEF name numArgs codeAddr) -> (code, stack', global, heap, t', p')
                                where
                                    t'      = t + 1
...
execUpdate numArgs (code, stack, global, heap, t, p) =
    case stack !! t of
        (C index) -> error "Something went wrong in execUpdate. C i entry not expected here."
        (H index) -> (code, stack, global, heap', t, p)
            where
...
execSlide n (code, stack, global, heap, t, p) = (code, stack', global, heap, t', p)
            where
...
execPushval (IntX x) (code, stack, global, heap, t, p) = (code, stack', global, heap', t', p)
                where
                    t' = t + 1
                    heap' = heap ++ [VAL (Int x)]
                    stack' = stack ++ [H (length heap' - 1)]

execPushval (BoolX x) (code, stack, global, heap, t, p) = (code, stack', global, heap', t', p)
                where
                    t' = t + 1
...
execReturn (code, stack, global, heap, t, p) =
    case stack !! (t-1) of
        C ix -> (code, stack', global, heap, t', p')
                where
                    p' = ix
...
execHalt (code, stack, global, heap, t, p)
...
execMakeapp (code, stack, global, heap, t, p) = (code, stack', global, heap', t', p)
                where
...
execPushparam n (code, stack, global, heap, t, p) = (code, stack', global, heap, t', p)
                where
                    t'      = t + 1
...
execOperator Negate (code, stack, global, heap, t, p) = 
    case stack !! t of 
        C n -> error "Something went wrong in execOperator Negate. Only H n is expected here."
        H n -> case heap !! n of
            VAL (Int x)     -> (code, stack', global, heap', t, p)
                where
...
execOperator Plus (code, stack, global, heap, t, p) = 
    case stack !! (t-1) of
        ...
        H n -> case stack !! t of
            ...
            H m -> (code, stack', global, heap', t', p)
                where
                    ...
                    stack'  = take (length stack - 2) stack ++ [H (length heap)]
                    heap'   = let (VAL (Int x1)) = heap !! n in let (VAL (Int x2)) = heap !! m in heap ++ [VAL $ Int (x1 - x2)]
execOperator Times (code, stack, global, heap, t, p) =
    case stack !! (t-1) of
        ...
        H n -> case stack !! t of
            ...
            H m -> (code, stack', global, heap', t', p)
                where
                    ...
                    stack'  = take (length stack - 2) stack ++ [H (length heap)]
                    heap'   = let (VAL (Int x1)) = heap !! n in let (VAL (Int x2)) = heap !! m in heap ++ [VAL $ Int (x1 * x2)]
execOperator DividedBy (code, stack, global, heap, t, p) =
    case stack !! (t-1) of
...
execOperator LessThan (code, stack, global, heap, t, p) =
    case stack !! (t-1) of
...
execOperator Equals (code, stack, global, heap, t, p) =
    case stack !! (t-1) of
...
execOperator And (code, stack, global, heap, t, p) = 
    case stack !! (t-1) of
...
execOperator Or (code, stack, global, heap, t, p) =
    case stack !! (t-1) of
...
execOperator If (code, stack, global, heap, t, p) =
    case stack !! (t-2) of
        C n -> error "Something went wrong in execOperator If. Only H n is expected here."
        H n -> case heap !! n of
            VAL (Bool True)    -> (code, stack', global, heap, t', p) -- THEN Case
                where
                    t'      = t - 2
                    stack'  = take (length stack - 3) stack ++ [stack !! (t - 1)]
            VAL (Bool False)    -> (code, stack', global, heap, t', p) -- ELSE Case
                where
                    t'      = t - 2
...
execAlloc (code, stack, global, heap, t, p) = (code, stack', global, heap', t', p)
                where
...
execSlideLet n (code, stack, global, heap, t, p) = (code, stack', global, heap, t', p)
                where
                    t' = t - n
